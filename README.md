...
- **CI 检查**：对 main 分支的 push / pull request 自动运行，依次执行 Bun 依赖安装、前端 Lint、构建以及数据库迁移冒烟测试，确保后端/前端的基础质量。
- **自动发布制品**：推送 `v*` 标签时触发，完成构建后将 `frontend/dist` 打包为 `frontend-dist.tar.gz`，并在 Release 中附带制品，方便部署与回溯。
